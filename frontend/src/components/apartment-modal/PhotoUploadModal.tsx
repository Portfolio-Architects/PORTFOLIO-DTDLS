      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-[#e5e8eb]">
          <h2 className="text-[18px] font-bold text-[#191f28] flex items-center gap-2">
            <Camera className="text-[#3182f6]" size={20} />
            우리 단지 사진 올리기
          </h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex flex-col gap-6">
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <h3 className="text-[20px] font-bold text-[#191f28] mb-2">등록이 완료되었습니다!</h3>
              <p className="text-[15px] text-[#4e5968]">관리자 검토 후 단지 갤러리에 반영됩니다.<br/>참여해 주셔서 감사합니다.</p>
            </div>
          ) : !user ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="w-16 h-16 bg-[#f2f4f6] rounded-full flex items-center justify-center mb-5">
                <Camera size={32} className="text-[#8b95a1]" />
              </div>
              <h3 className="text-[18px] font-bold text-[#191f28] mb-2">로그인이 필요합니다</h3>
              <p className="text-[14px] text-[#4e5968] mb-6">
                단지 사진을 등록하려면<br />구글 로그인을 먼저 진행해 주세요.
              </p>
              <button
                onClick={() => signInWithPopup(auth, googleProvider)}
                className="flex items-center gap-2 bg-white border border-[#e5e8eb] shadow-sm text-[#191f28] font-bold px-6 py-3 rounded-xl hover:bg-[#f9fafb] transition-colors"
              >
            ...
            <>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[14px] font-bold text-[#4e5968]">사진 등록 (필수)</label>
                <div 
                  className={`w-full aspect-video rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-colors relative overflow-hidden
                    ${previewUrl ? 'border-transparent bg-black' : 'border-[#d1d6db] bg-[#f8f9fa] hover:bg-[#f2f4f6]'}`}
                ...
                >
                  {previewUrl ? (
                    ...
                    <div className="flex flex-col items-center gap-3 text-[#8b95a1] p-6 text-center">
                      <div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center">
                        <ImageIcon size={24} className="text-[#3182f6]" />
                      </div>
                      <p className="text-[14px] font-medium">여기를 눌러 사진을 선택하세요<br/><span className="text-[12px] text-[#b0b8c1]">(최대 10MB)</span></p>
                    </div>
                  )}
                  <input 
                    type="file" 
              ...
              {/* Category */}
              <div className="flex flex-col gap-2">
                <label className="text-[14px] font-bold text-[#4e5968]">카테고리 (필수)</label>
                    ...
                    <button
                      ...
                      className={`py-2 px-1 text-[13px] font-bold rounded-xl transition-colors
                        ${category === cat.id 
                          ...
                          : 'bg-[#f2f4f6] text-[#4e5968] border border-transparent hover:bg-[#e5e8eb]'}`}
                    >
                      {cat.label}
                    </button>
                  ))}
              ...
              {/* Caption */}
              <div className="flex flex-col gap-2">
                <label className="text-[14px] font-bold text-[#4e5968]">간단한 설명 (선택)</label>
                <textarea
                  placeholder="예: 단지 내 메인 산책로 야경입니다."
                  value={caption}
                  onChange={(e) => setCaption(e.target.value)}
                  className="w-full bg-[#f2f4f6] border-none rounded-xl p-4 text-[15px] text-[#191f28] placeholder:text-[#8b95a1] focus:ring-2 focus:ring-[#3182f6]/30 focus:bg-white transition-all resize-none h-24"
                  maxLength={100}
                />
                <div className="text-right text-[12px] text-[#8b95a1] font-medium">
                  {caption.length}/100
                </div>
              </div>
            </>
        ...
        {/* Footer */}
        {(!isSuccess && user) && (
          <div className="p-5 border-t border-[#e5e8eb] bg-white">
            <button
              onClick={handleSubmit}
              disabled={!file || isUploading}
              className={`w-full py-4 rounded-xl text-[16px] font-bold flex items-center justify-center gap-2 transition-all
                ${(!file || isUploading) ? 'bg-[#f2f4f6] text-[#b0b8c1] cursor-not-allowed' : 'bg-[#3182f6] text-white hover:bg-[#1b64da] shadow-[0_4px_12px_rgba(49,130,246,0.2)]'}`}
            >
              {isUploading ? (
                <>
