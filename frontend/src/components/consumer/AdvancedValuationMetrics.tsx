...
export default function AdvancedValuationMetrics({ report, transactions }: Props) {
    ...
    const fetchMacroRates = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (json.success && json.data && json.data.riskFreeRate) {
            setMacroConfig(prev => ({
              ...prev,
              riskFreeRate: json.data.riskFreeRate,
            }));
          }
        }
      } catch (err) {
  ...
  const dcf = calculateDynamicDCF(avg3MRent, macroConfig);
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <div className="flex flex-col items-center justify-center pb-6">
                ...
                <div className="text-[14px] font-bold text-toss-blue mt-3 bg-toss-blue/10 px-4 py-1.5 rounded-full border border-toss-blue/20 shadow-sm">
                  적용 요구수익률(Cap Rate) {dcf.capRate.toFixed(2)}%
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-xl border flex gap-3 items-start bg-body border-border">
                ...
                <div className="flex flex-col gap-1.5">
                  <h4 className="text-[13px] font-extrabold text-secondary">어떻게 계산했나요?</h4>
                  <p className="text-[11.5px] text-tertiary leading-relaxed font-medium">
                    현재 은행 금리(국채 및 대출금리)와 해당 단지의 전월세 시세를 바탕으로, 월세 수익 가치를 역산해 낸 이론적인 적정 매매가입니다.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex items-center gap-1.5 text-[11px] text-[#8b95a1] bg-[#f9fafb] px-2 py-1.5 rounded-md mt-0.5 border border-[#e5e8eb]/50">
                      ...
                      <span>리스크 1.50%</span>
                      <span className="text-[#d1d6db]">+</span>
                      <span>조달금리 페널티 {(Math.max(0, macroConfig.fundingCost - 4.0) * 0.5).toFixed(2)}%</span>
                    </div>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-[13px] text-tertiary font-medium">기대 성장률 (g: 장기 인플레)</span>
                    <span className="text-[13px] font-bold text-primary">{(macroConfig.baseInflationRate * 100).toFixed(2)}%</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <span className="text-[13px] font-bold text-primary">{formatPrice(avg3MRent * macroConfig.jeonseConversionRate)}</span>
                  </div>
