...
const NetflixCard = ({ cat, apt, txSummary, report, rank, onClick }: any) => {
  ...
  return (
    ...
    >
      ...
      {/* Top Left Rank */}
      {rank != null && (
        <div className="absolute top-2 left-2 md:top-3 md:left-3 bg-black/60 backdrop-blur-md rounded-full px-2 py-0.5 md:px-3 md:py-1 z-10 border border-white/10 shadow-lg">
          <span className="text-white font-extrabold text-[12px] md:text-[14px] italic">{rank}위</span>
        </div>
      )}
      
      {/* Base Content */}
      <div className={`absolute bottom-0 left-0 right-0 z-10 flex flex-col p-3 md:p-5 gap-0.5 md:gap-1 transition-transform duration-300 group-hover:-translate-y-1`}>
        <h4 className={`${titleColor} font-extrabold leading-tight line-clamp-2 break-keep text-[16px] md:text-[20px]`} style={hasImage ? { textShadow: "0 2px 8px rgba(0,0,0,0.5)" } : {}}>
          {apt.name}
        </h4>
        <div className="flex items-center gap-1.5 md:gap-2 mt-0.5">
          <span className={`${priceColor} font-extrabold text-[14px] md:text-[16px]`}>{priceText}</span>
