...
- [ ] **Zero-Jank UX & Main Thread Unblocking (React 18 Concurrent)**: 180+개 아파트 정렬 시 발생하는 O(N²) Array 탐색을 O(1) Hash Map으로 사전 연산하고, `startTransition` 기반 비동기 탭 전환 및 뷰포트 가상화를 적용하여 UI 버벅임(Stuttering) 완벽 제거.<br><br>**[Utility and Risk Bayesian Inference]**<br>- **Prior:** 방대한 DOM 렌더링 및 동기적 Array 연산에 의한 메인 스레드 블로킹으로 탭 전환 시 모바일 환경 화면 멈춤(Freezing) 발생 확률 **~85%**.<br>- **Likelihood:** Hash Map Caching과 `react-window`를 결합하고, React 18 Concurrent Rendering(`useTransition`)을 도입하면 연산 병목을 해소하고 렌더링을 백그라운드로 미루어 브라우저 페인팅 사이클을 보장할 수 있음.<br>- **Utility:** 네이티브 앱 수준의 120fps 부드러운 스크롤 및 즉각적인 탭 전환(Zero-Jank) 달성을 통해 사용자 이탈 방지 및 체류 시간 극대화.<br>- **Posterior Risk:** 비동기 렌더링 지연으로 인한 일시적인 빈 화면(Skeleton/Pending) 노출. 낙관적 UI 업데이트 및 Shimmer 로딩 애니메이션 동반 적용으로 체감 대기 시간 리스크를 **< 2%**로 억제.
